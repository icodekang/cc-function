...
;(async () => {
  try {
    ...
    const files = await readMarkdownPaths(path.resolve(process.cwd(), 'function'));
    ...
    await FS.outputFile(path.resolve(deployDir, 'js', 'dt.js'), `var cc_functions=${JSON.stringify(jsonData.data)}`);
    ...
    await createTmpToHTML(
      ...
      path.resolve(deployDir, 'index.html'),
      {
        p: '/index.html',
        n: 'C/C++函数搜索引擎',
        d: 'C/C++函数大全，内容包含C++标准库以及值得收藏的功能函数，是一个值得收藏的C/C++函数查询手册。',
        function_length: jsonData.data.length
      }
    );
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (mdPath) {
        // CSS/JS 引用相对地址
        relative_path = '../';
        mdPathName = `/function/${desJson.n}.md`;
